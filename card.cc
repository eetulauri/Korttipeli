...
void Card::setupFront()
{
    ...
    // kortin yläosan kuva ja numero
    painter.drawPixmap(13, 5, suitPixmap_, 0, 0, 0, 0);

    painter.drawText(frontPixmap.width()/2-42, 0, suitPixmap_.width() + 5,
                     suitPixmap_.height() + 5, Qt::AlignCenter,
                     QString::number(value_));

    // alaosan kuva ja numero
    painter.drawPixmap(frontPixmap.width()-38,
                       frontPixmap.height()-suitPixmap_.height()-5,
                       suitPixmap_, 0, 0, 0, 0);

    painter.drawText(frontPixmap.width()/2 + 18, 0, suitPixmap_.width() + 5,
                     459, Qt::AlignCenter, QString::number(value_));
